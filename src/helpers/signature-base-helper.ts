...
function generateBase(headers: any, config: Config): string {
  try {
    ...
    signatureParams.forEach((header) => {
      if (
        header === constants.HEADERS.CONTENT_DIGEST &&
...
function calculateBase(headers: any, config: Config): string {
  try {
    const signatureInputHeader: string = headers[
      constants.HEADERS.SIGNATURE_INPUT
    ] as string;
    ...
    const signatureInputPattern = new RegExp(
      ".+=(\\((.+)\\);created=(\\d+)(;keyid=.+)?)",
    );

    const signatureInputParts =
      signatureInputPattern.exec(signatureInputHeader);
